...
public class PlaceTypePanel extends JPanel {
	 ...
	 */
	public PlaceTypePanel(JRootPane _rootPane, Place _place, Vector<DataType> _group, DataLayer _pnmlData,
			GuiView _view) {
	...
	private void initListeners() {
		...
		// timeUpperBoundButton.addActionListener(new
	...
	private void showDefinedDataType() {
		...
		if (dataType.getDef()) {
			...
			isTypeCheckBox.setSelected(place.getDataType() == null
					|| this.dataType.getName().equals(place.getDataType().getName().trim()));
		}
	}
	...
	private void initComponents() {
		...
		timeLowerBoundButton.setText("ArrivingTime");
		...
		// mDatatypePoolUI.setSize(TypeTextField.getWidth()
		// ,TypeTextField.getHeight());
		gridBagConstraints.gridx = 0;
		gridBagConstraints.gridy = 0;
	...
	// private void markingComboBoxActionPerformed(java.awt.event.ActionEvent
	// evt) {//GEN-FIRST:event_markingComboBoxActionPerformed
	// Integer index = markingComboBox.getSelectedIndex();
	//
	// if (index > 0){
	// Integer value =
	// ((MarkingParameter)markingComboBox.getItemAt(index)).getValue();
	...
	private void doOK() {
		...
		if ((StringUtils.isNotBlank(type) || StringUtils.isNotBlank(name)) && dataType == null || // no
																									// datatype
																									// has
																									// been
																									// created
																									...
																									// unfinished
				dataType != null
						&& (!dataType.getStringRepresentation().equals(type) || !dataType.getName().equals(name))) {
			userResponse = JOptionPane.showConfirmDialog(this.getParent(),
					"You have changed/created the datatype. Do you want to apply the changes?", "",
					JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
		}

		boolean isSuccessful = true; // by default true. Will be false only if
										// the user chooses to save but failed
										// due validation check.
	...
	private void timeLowerBoundButtonHandler(ActionEvent evt) {
		includeIntermediateDefinitionParts("arrivingTime");
	}
