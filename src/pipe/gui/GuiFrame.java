...
public class GuiFrame extends JFrame implements ActionListener, Observer {
  ...
  private TypeAction annotationAction, arcAction, bidirectionalAction, inhibarcAction, placeAction,
...
//      this.setIconImage(new ImageIcon(GuiFrame.class.getResource("/Images/icon.png")).getImage());
...
//      exportMenu.setIcon(new ImageIcon(GuiFrame.class.getResource("/Images/Export.png")));
    addMenuItem(exportMenu, exportPNGAction = new FileAction("PNG", "Export the net to PNG format", "ctrl G"));
    addMenuItem(exportMenu, exportPSAction = new FileAction("PostScript", "Export the net to PostScript format", "ctrl T"));
...
//      }
    ...
    addMenuItem(drawMenu, transAction = new TypeAction("Immediate transition", Pipe.IMMTRANS, "Add an immediate transition", "I", true));
    addMenuItem(drawMenu, timedtransAction = new TypeAction("Timed transition", Pipe.TIMEDTRANS, "Add a timed transition", "T", true));
    addMenuItem(drawMenu, arcAction = new TypeAction("Arc", Pipe.ARC, "Add an arc", "A", true));
    addMenuItem(drawMenu, bidirectionalAction = new TypeAction("Bidirectional Arc", Pipe.BIDIRECTARC, "Add bidirectional arc", "B", true));
    addMenuItem(drawMenu, inhibarcAction = new TypeAction("Inhibitor Arc", Pipe.INHIBARC, "Add an inhibitor arc", "H", true));
    addMenuItem(drawMenu, annotationAction = new TypeAction("Annotation", Pipe.ANNOTATION, "Add an annotation", "N", true));
    drawMenu.addSeparator();
    addMenuItem(drawMenu, tokenAction = new TypeAction("Add token", Pipe.ADDTOKEN, "Add a token", "ADD", true));
    addMenuItem(drawMenu, deleteTokenAction = new TypeAction("Delete token", Pipe.DELTOKEN, "Delete a token", "SUBTRACT", true));
    drawMenu.addSeparator();
    addMenuItem(drawMenu, rateAction = new TypeAction("Rate Parameter", Pipe.RATE, "Rate Parameter", "R", true));
    addMenuItem(drawMenu, markingAction = new TypeAction("Marking Parameter", Pipe.MARKING, "Marking Parameter", "M", true));
...
//      zoomMenu.setIcon(new ImageIcon(GuiFrame.class.getResource("/Images/Zoom.png")));
    ...
    addMenuItem(animateMenu, startAction = new AnimateAction("Animation mode", Pipe.START, "Toggle Animation Mode", "Ctrl A", true));
    animateMenu.addSeparator();
    addMenuItem(animateMenu, stepbackwardAction = new AnimateAction("Back", Pipe.STEPBACKWARD, "Step backward a firing", "4"));
    addMenuItem(animateMenu, stepforwardAction = new AnimateAction("Forward", Pipe.STEPFORWARD, "Step forward a firing", "6"));
    addMenuItem(animateMenu, randomLowLevelAction = new AnimateAction("RandomLowLevel", Pipe.RANDOMLOWLEVEL, "Randomly fire a low level transition", "5"));
    addMenuItem(animateMenu, randomHighLevelAction = new AnimateAction("RandomHighLevel", Pipe.RANDOMHIGHLEVEL, "Randomly fire a high level transition", "6"));
    addMenuItem(animateMenu, randomAnimateAction = new AnimateAction("Animate", Pipe.ANIMATE, "Randomly fire a number of transitions", "7", true));
    addMenuItem(animateMenu, modelCheckingAction = new AnimateAction("ModelChecking", Pipe.MODELCHECKING, "Model checking Petri nets", "8", true));
    addMenuItem(animateMenu, modelCheckingActionUsingMaude = new AnimateAction("ModelCheckingUsingMaude", Pipe.MODELCHECKING_MAUDE, "Model checking of Petri nets using Maude", "9", true));
    addMenuItem(animateMenu, convertZ3Action = new AnimateAction("Z3", Pipe.CONVERTZ3, "Convert Petri Nets model to Z3 SMT", "10", true));
...
//    		  GuiFrame.class.getResource("/Images/About.png");
    ...
    addMenuItem(experimentMenu, loadExperimentAction = new ExperimentAction("Load experiment", "Load an experiment file", ""));
    addMenuItem(experimentMenu, experimentEditorAction = new ExperimentAction("Experiment Editor", "Start the experiment editor", ""));
      ...
      switch (typeID) {
        case Pipe.START:
          ...
          if (!ck.getResult() && !appView.isInAnimationMode()) {
            JOptionPane.showMessageDialog(GuiFrame.this, ck.getMessage(),
                "Model Completeness Check Error", JOptionPane.ERROR_MESSAGE);
          }
          try {
...
//          animBox.clearStepsForward();
          CreateGui.getAnimator().startAnimation(new FixedStepsAnimationStrategy(1), CreateGui.getAnimationHistory(appTab.getSelectedIndex()));
//          stepforwardAction.setEnabled(animBox.isStepForwardAllowed());
//          stepbackwardAction.setEnabled(animBox.isStepBackAllowed());
          break;
...
//          animBox.clearStepsForward();
          ...
          propertyChangeSupport.addPropertyChangeListener(new PropertyChangeListener() {
            ...
            public void propertyChange(final PropertyChangeEvent evt) {
              ...
              if (newValue != null && newValue instanceof AnimationStrategy) {
                a.startAnimation((AnimationStrategy) newValue, CreateGui.getAnimationHistory(appTab.getSelectedIndex()));
              }
              else {
                a.stopAnimation();
              }
    ...
    private void showModelCheckingOptionsWithSpin() {
      ...
      final JComboBox<String> optionBox = new JComboBox<>(new String[]{"_Select_", "Default", "Transition as Proctype"});
