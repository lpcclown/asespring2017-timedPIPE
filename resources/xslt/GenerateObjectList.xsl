...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   ...
   <xsl:template match="net">
      ...
      <xsl:apply-templates select="sysPlace"/>
   ...
   <xsl:template name="agentNetTemplate">
      ...
      <xsl:apply-templates select="agentNet/definition"/>
      ...
      <xsl:apply-templates select="agentNet/page"/>
      <xsl:apply-templates select="agentNet/module"/>       
   	  </xsl:element>
   </xsl:template>  
...
<!--         <xsl:attribute name="parameter">
		...
		<xsl:for-each select = "Datatype-group/DataType">
      ...
      <xsl:template name = "group-DataType">
		<xsl:element name = "group-DataType">
			<xsl:attribute name="datatype-name">
            <xsl:value-of select="name/value"/>
			</xsl:attribute>
			<xsl:attribute name="datatype-Ntype">
				<xsl:value-of select="Ntype/value"/>
			</xsl:attribute>
			<xsl:attribute name="datatype-types">
				<xsl:value-of select="types/value"/>
			</xsl:attribute>
			<xsl:attribute name="datatype-ifPow">
				<xsl:value-of select="ifPow/value"/>
			</xsl:attribute>
			<xsl:attribute name="datatype-NumofElement">
				<xsl:value-of select="NumofElement/value"/>
			</xsl:attribute>
			<xsl:attribute name="datatype-isDef">
				<xsl:value-of select="isDef/value"/>
			</xsl:attribute>
		</xsl:element>
   </xsl:template>
   ...
   <xsl:template name = "DataType">
		<xsl:attribute name="datatype-name">
            <xsl:value-of select="DataType/name/value"/>
        </xsl:attribute>
		<xsl:attribute name="datatype-Ntype">
            <xsl:value-of select="DataType/Ntype/value"/>
        </xsl:attribute>
		<xsl:attribute name="datatype-types">
            <xsl:value-of select="DataType/types/value"/>
        </xsl:attribute>
		<xsl:attribute name="datatype-ifPow">
            <xsl:value-of select="DataType/ifPow/value"/>
        </xsl:attribute>
		<xsl:attribute name="datatype-NumofElement">
            <xsl:value-of select="DataType/NumofElement/value"/>
        </xsl:attribute>
		<xsl:attribute name="datatype-isDef">
            <xsl:value-of select="DataType/isDef/value"/>
        </xsl:attribute>
   </xsl:template>
   
   <xsl:template name = "abTokentemplate">
		<xsl:attribute name = "isDef">
			<xsl:value-of select= "abtoken/isDef/value"/>
		</xsl:attribute>
		<xsl:for-each select = "abtoken/listToken/token">
			<xsl:element name = "token">
				<xsl:for-each select = "data">
					<xsl:element name = "data">
						<xsl:attribute name = "token-data">
...
<!--         <xsl:attribute name="parameter">
   ...
   <xsl:template match="interface">
	   ...
	   <xsl:template match="agentNet/page">
      <xsl:apply-templates select="labels"/>
      <xsl:apply-templates select="definition"/>
      <xsl:apply-templates select="place"/>
      <xsl:apply-templates select="transition"/>
   ...
   <xsl:template match="agentNet/module">
      <xsl:apply-templates select="labels"/>
      <xsl:apply-templates select="definition"/>
      <xsl:apply-templates select="place"/>
      <xsl:apply-templates select="transition"/>
   ...
   <xsl:template match="agentNet/definition">
      <xsl:element name="definition">
         <xsl:attribute name="positionX">
            <xsl:value-of select="graphics/position/@x"/>
         </xsl:attribute>
